  ...
  const script = createVideoScript()
    ...
    .segment("Here's the detail page for ReActor Node — I can see the name, publisher badge, version, and download count right at the top.", {
      setup: async () => {
        ...
        await page.waitForFunction(() => document.body && document.body.innerText.trim().length > 200, { timeout: 20000 }).catch(() => {});
        await page.waitForTimeout(2000);
      },
      action: async (pace) => {
        await safeMove(page, "h1");
    ...
    // ✅ view_readme — PASS
    .segment("Scrolling down through the node's documentation, release notes, and version history — all rendered on this page.", async (pace) => {
      await page.mouse.wheel(0, 300);
      await page.waitForTimeout(500);
      await safeMove(page, "article, .prose, [class*='readme'], [class*='markdown'], h2, h3");
      await page.mouse.wheel(0, 300);
      await pace();
    })
    // ✅ view_install_command — PASS
    .segment("Here's the install command — I'll click the copy button to grab it for my terminal.", async (pace) => {
      await page.mouse.wheel(0, -300);
      await page.waitForTimeout(400);
      await safeMove(page, "code, pre, [class*='install'], [class*='command']");
      // Click the copy button if present, otherwise click the code block
      await page.locator("button[aria-label*='copy' i], button:has-text('Copy'), [class*='copy']").first().click({ timeout: 3000 }).catch(async () => {
        await page.locator("code, pre").first().click({ timeout: 3000 }).catch(() => {});
      });
      await page.waitForTimeout(400);
      await pace();
    })
    ...
    // ✅ view_publish_guide — PASS
    .segment("The sidebar has a link to the publishing guide — clicking it takes me to the step-by-step instructions for packaging and publishing custom nodes to the registry.", async (pace) => {
      await safeMove(page, "nav, aside");
      await page.locator("a[href*='publish']").first().click({ timeout: 3000 }).catch(() => {});
      await page.waitForFunction(() => document.body && document.body.innerText.trim().length > 200, { timeout: 15000 }).catch(() => {});
      await page.waitForTimeout(1000);
      await safeMove(page, "h1");
      await page.mouse.wheel(0, 200);
      await pace();
    ...
    // Final scorecard
    .segment("Here are the final QA results for Comfy Registry. Out of 14 operations, 11 passed — 79 percent. Landing Page, Node Search, and Documentation work fully. Node Detail scored 4 out of 5, and Publisher Profile scored 0 out of 2 — publisher profile pages are not yet available.", {
      setup: async () => {
    ...
    .outro({
      text: "QA Results: 11/14 (79%)",
      subtitle: "Landing Page 2/2 ✅ | Node Search 3/3 ✅ | Node Detail 4/5 ⚠ | Publisher Profile 0/2 ⚠ | Documentation 2/2 ✅",
      durationMs: 4000,
    });
